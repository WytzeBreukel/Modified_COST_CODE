...
fn main() {
    ...
    match mode.as_str() {
        "vertex" => {
            union_find_modified(&NodesEdgesMemMapper::new(&name), nodes)
        },
        "hilbert" => {
            union_find_modified(&UpperLowerMemMapper::new(&name), nodes)
        },
        "compressed" => {
            union_find_modified(&DeltaCompressedReaderMapper::new(|| BufReader::new(File::open(&name).unwrap())), nodes)
        },
        _ => { println!("unrecognized mode: {:?}", mode); },
    }
}
fn union_find_modified<G: EdgeMapper>(graph: &G, nodes: u32) {
